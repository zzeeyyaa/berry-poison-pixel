...
import { fetchProductsAction, deleteProductAction } from "@/src/actions/products";
import { fetchCategoriesAction, fetchAllCategoriesAction, deleteCategoryAction } from "@/src/actions/categories";
...
export default function ManagementPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const { products: prodData, total } = await fetchProductsAction(
        productPage,
        PAGE_SIZE,
        searchQuery,
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const { categories: catData, total } = await fetchCategoriesAction(categoryPage, PAGE_SIZE);
      setCategoryTotal(total);
      setPaginatedCategories(catData);
    } catch (err: any) {
      console.error(err);
        ...
        <div className="flex justify-center mb-2">
          <div className="flex w-full sm:w-auto gap-1 p-1.5 bg-[#FCF8F7] border border-[#4E3C44]/10 rounded-xl shadow-inner relative z-10">
            <button
              ...
              className={`flex-1 sm:flex-none px-3 py-2 sm:px-6 sm:py-2.5 font-bold transition-all duration-300 text-[10px] sm:text-xs rounded-lg flex justify-center items-center gap-1.5 sm:gap-2 ${activeTab === "products"
            ...
            <button
              ...
              className={`flex-1 sm:flex-none px-3 py-2 sm:px-6 sm:py-2.5 font-bold transition-all duration-300 text-[10px] sm:text-xs rounded-lg flex justify-center items-center gap-1.5 sm:gap-2 ${activeTab === "categories"
